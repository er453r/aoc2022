fun main() {
    class Node(val name:String, val rate:Int, var paths:List<Node> = emptyList())
    ...
    fun parseData(input: List<String>):Map<String, Node>{
        ...
        for(line in input)
            line.split(";").first().destructured(inputLineRegex).let { (name, rate) -> nodes[name] = Node(name, rate.toInt()) }
